...
pub async fn collect_events(
    ...
    let mut hash_nodes = HashMap::<(String, Hash), codegen::Node>::new();
    ...
    for (input_name, input) in inputs {
        for (rel_path, web_asset) in &input.web {
            input_sources.entry(input_name.clone()).or_default().insert(
                rel_path.clone(),
                codegen::asset_ref_node(&web_asset.clone().into()),
            );
        }
    }
            ...
            super::Event::Finished {
                ...
                match state {
                    super::EventState::Duplicate => {
                        ...
                        if let Some(node) = hash_nodes.get(&key) {
                            input_sources
                                .entry(input_name.clone())
                                .or_default()
                                .insert(rel_path.clone(), node.clone());
                        } else {
                            pending_duplicates
                                .entry(key)
                                .or_default()
                    ...
                    super::EventState::Synced { new } => {
                        if let Some(asset_ref) = asset_ref {
                            ...
                            let node = node
                                .map(|node| *node)
                                .unwrap_or_else(|| codegen::asset_ref_node(&asset_ref));
                            hash_nodes.insert(key.clone(), node.clone());

                            let source = input_sources.entry(input_name.clone()).or_default();
                            source.insert(rel_path.clone(), node.clone());

                            if let Some(rel_paths) = pending_duplicates.remove(&key) {
                                for rel_path in rel_paths {
                                    source.insert(rel_path, node.clone());
                                }
                            }
